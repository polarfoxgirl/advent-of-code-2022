...
pub fn solve() {
    ...
    let abyss = rock.iter().map(|(_, y)| *y).max().unwrap();

    let mut sand_at_rest: HashSet<(u32, u32)> = HashSet::new();
    loop {
        match drop_sand(&abyss, &rock, &sand_at_rest) {
            None => break,
            Some(sand) => {
...
fn drop_sand(abyss: &u32, rock: &HashSet<(u32,u32)>, sand_at_rest: &HashSet<(u32, u32)>) -> Option<(u32, u32)> {
    let mut sand = (500, 0);
    loop {
        match move_sand(rock, sand_at_rest, &sand) {
...
fn move_sand(rock: &HashSet<(u32,u32)>, sand_at_rest: &HashSet<(u32, u32)>, sand: &(u32, u32)) -> Option<(u32, u32)> {
