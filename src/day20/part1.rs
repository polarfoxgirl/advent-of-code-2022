...
fn mix_list(list: &mut Vec<(usize, i32)>) -> () {
    let length = list.len();
    let int_length = i32::try_from(length).unwrap();
...
fn find_item_pos<'a>(list: &'a Vec<(usize, i32)>, next: &usize, start_pos: &usize) -> (&'a i32, usize) {
...
fn calc_target_pos(item: &i32, pos: &usize, int_length: &i32) -> usize {
    let mut int_value = (i32::try_from(*pos).unwrap() + *item) % (*int_length - 1);
...
fn move_item(list: &mut Vec<(usize, i32)>, pos: &usize, target_pos: &usize) -> () {
    if *pos == *target_pos {
        return;
    }
...
fn get_coordinates(list: &Vec<(usize, i32)>) -> (i32, i32, i32) {
...
fn find_zero_pos(list: &Vec<(usize, i32)>) -> usize {
    let mut pos = 0;
    loop {
...
fn read_input(filename: String) -> Vec<(usize, i32)> {
