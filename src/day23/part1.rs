...
fn calc_space(elves: &HashSet<(i32, i32)>) -> i32 {
    let (min_x, min_y, max_x, max_y) = get_rectangle(elves);

    (max_x + 1 - min_x) * (max_y + 1 - min_y) - i32::try_from(elves.len()).unwrap()
}
...
fn do_round(elves: &mut HashSet<(i32, i32)>, round: &usize) -> () {
